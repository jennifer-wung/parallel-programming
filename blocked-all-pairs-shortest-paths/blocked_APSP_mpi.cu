...
#define CMCPYDTD cudaMemcpyDeviceToDevice
...
int* combineData(int numRound, int r, int* src_d, int* recv_d, int rank) 
{
	int BLOCK_FACTOR = THREAD_WIDTH*BLOCK_WIDTH;
	if (rank == 1) { // only did the lower half 
...
template <int BLOCK_FACTOR> __global__ void cal_phase_two(const unsigned int Round, const unsigned int n, const size_t pitch, int * const d, const unsigned int rank)
{
...
template <int BLOCK_SIZE, int THREAD_SIZE> __global__ void cal_phase_three(const unsigned int Round, const unsigned int n, const size_t pitch, int * const d, int rank) 
{
	if(blockIdx.x == Round || blockIdx.y == Round) return;
	if(rank == 0)
		if(blockIdx.y > Round) return;//do upper half
	else
		if(blockIdx.y < Round) return;//do lower half
	int newPath;
	int path;
...
template <int BLOCK_SIZE, int THREAD_SIZE> void block_APSP(const unsigned int rank)
{
	int *dev_d = 0;
	int *host_d_recv = 0;
	int *host_d = 0;
	...
	cudaMallocHost( (void **) &host_d, n*n*sizeof(int));
	cudaMallocHost( (void **) &host_d_recv, n*n*sizeof(int));
	...
	for(int r = 0; r < round; r++) { 
		...
		cal_phase_two<BLOCK_FACTOR><<<dimGridP2, dimBlockP2>>>(r, n, pitch_int, dev_d,rank);
		cal_phase_three<BLOCK_SIZE, THREAD_SIZE><<<dimGridP3, dimBlockP3>>>(r, n, pitch_int, dev_d,rank);
		
		cudaStatus = cudaMemcpy2D(host_d, n*sizeof(int), dev_d, pitch, n*sizeof(int), n, CMCPYDTH);
		cudaStatus = cudaDeviceSynchronize();
		MPI_Sendrecv(host_d, n*n, MPI_INT, MPI_RECIEVER, 0, host_d_recv, n*n, MPI_INT, 
									MPI_RECIEVER, 0, MPI_COMM_WORLD, &status);
		host_d = combineData(round,r,host_d,host_d_recv,rank);
		cudaMemcpy2DAsync(dev_d, pitch, host_d, n*sizeof(int), n*sizeof(int), n, CMCPYHTD, cpyStream);
	}
...
//	printf("same_complex? %d\n",same_complex);
	...
	block_APSP<BLOCK_WIDTH, THREAD_WIDTH>(rank);
